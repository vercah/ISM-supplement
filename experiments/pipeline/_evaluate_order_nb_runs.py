...
def compute_changes(d, order, num_rows):
    ...
    n_cylinder_runs = n_changes_path + get_distance(d, order[0], order[len(order)-1])
    return n_bit_changes, n_runs, n_cylinder_runs

def main():
    parser = argparse.ArgumentParser(description="Compute runs from distance matrix and genome order")
    parser.add_argument("dist_fn", help="distance file (pairwise distances)")
    parser.add_argument("order_fn", help="genome order file")
    parser.add_argument("--num-rows", type=int, required=True, help="Number of rows in the original matrix")
    ...
    n_changes, n_runs, n_cylinder_runs = compute_changes(d, order, args.num_rows)
